...
function getNextPokemonId() {
    $ids = array_map(fn($p) => $p->getId(), $_SESSION['pokemons']);
    sort($ids);
    $expected = 1;
    foreach ($ids as $id) {
...
function updatePokemon($id, $name, $weight, $species) {
    foreach ($_SESSION['pokemons'] as $p) {
        if ($p->getId() == (int)$id) {
            $p->setName(trim($name));
            $p->setWeight(floatval($weight));
            $p->setSpecies(trim($species));
            break;
        }
    }
}
...
function addRelation($pokemon_id, $type_id) {
    $foundP = false; $foundT = false;
    foreach ($_SESSION['pokemons'] as $p) if ($p->getId() == (int)$pokemon_id) $foundP = true;
    foreach ($_SESSION['types'] as $t) if ($t->getId() == (int)$type_id) $foundT = true;
    if ($foundP && $foundT) {
        foreach ($_SESSION['pokemons'] as $p) {
            if ($p->getId() == (int)$pokemon_id) { $p->setTypeId((int)$type_id); break; }
        }
        return true;
    }
    return false;
